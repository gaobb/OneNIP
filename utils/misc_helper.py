...
def save_checkpoint(state, is_best, config):
    ...
    torch.save(state, os.path.join(folder, "ckpt.pkl"))
    if is_best:
        shutil.copyfile(
            os.path.join(folder, "ckpt.pkl"),
            os.path.join(folder, "ckpt_best.pkl"),
        )
    ...
    if config.saver.get(
        ...
        shutil.copyfile(
            os.path.join(folder, "ckpt.pkl"),
            os.path.join(folder, f"ckpt_{epoch}.pkl"),
        )
...
def load_state(path, model):
    ...
    if os.path.isfile(path):
        ...
        #torch.save(state, os.path.join(path.replace("ckpt.pth.tar", "ckpt.pkl")))
...
def update_config(config):
    ...
    if reconstruction_type == "UniAD"  or reconstruction_type == "OneNIP":
        if isinstance(config.dataset.input_size, str):
            input_size = eval(config.dataset.input_size)
            config.dataset.input_size = input_size
        outstride = config.net[1].kwargs.outstrides[0]
        if isinstance(config.net[2].kwargs.neighbor_mask.mask, str): 
            config.net[2].kwargs.neighbor_mask.mask = eval(config.net[2].kwargs.neighbor_mask.mask) 
        assert (
        ...
        neighbor_size = [s // 2 for s in feature_size]
        config.net[2].kwargs.neighbor_mask.neighbor_size = neighbor_size
