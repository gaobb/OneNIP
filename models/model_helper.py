...
class ModelHelper(nn.Module):
    ...
    def forward(self, input):
        ...
        if "prompt" in input and "aimage" not in input:
            input["image"] = torch.cat((input["image"], input["prompt"]), dim = 0)

        if "aimage" in input and "prompt" in input:
            input["image"] = torch.cat((input["image"], input["aimage"], input["prompt"]), dim = 0)
            input["mask"] = torch.cat((input["mask"], input["amask"]), dim =0)
