...
def main():
    ...
    # update save dir
    config.saver.load_path =  os.path.join(config.saver.save_dir, args.class_name, 'ckpt.pkl')
    config.saver.log_dir = os.path.join(config.saver.save_dir, args.class_name, 'log')
    config.evaluator.save_dir = os.path.join(config.saver.save_dir, args.class_name, 'result_eval_temp')
    config.evaluator.vis_compound.save_dir = os.path.join(config.saver.save_dir, args.class_name, 'vis_compound')
    config.net[2].kwargs.save_recon.save_dir = os.path.join(config.saver.save_dir, args.class_name, 'result_recon')
    ...
    train_loader, val_loader = build_dataloader(config.dataset, args.class_name, distributed=True)
...
def validate(val_loader, model):
    ...
    if rank == 0:
        ...
        # evaluate, log & vis
        ret_metrics = performances(fileinfos, rec_preds, masks, config.evaluator.metrics)
